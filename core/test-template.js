const loginNumber = {LOGIN_NUMBER};
                        ...
                        if ($highestBidder.length) {
                            cy.get('.text-truncate.final-price').invoke('text').then(priceText => {
                                ...
                                if (price !== parseFloat(amountValue)) {
                                    ...
                                    Cypress.log({ name: 'error', message: `Price mismatch: Amount Value: ${amountValue}, Final Price: ${price}` });
                                }
                            });
                        } else {
                            console.log('Highest Bidder does not exist');
                        }
                    });
                });               
